...
def test_train_step_area_importance_sampling_reports_band_volume() -> None:
    class PlaneModel(torch.nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.bias = torch.nn.Parameter(torch.tensor(0.0, dtype=torch.float32))
    ...
    torch.manual_seed(4)
    batch, _ = _build_batch()
    area_points = torch.zeros((2, 2, 3), dtype=batch["query_points"].dtype)
    batch["query_points"] = torch.cat([batch["query_points"].detach(), area_points], dim=1)
    batch["query_group"] = torch.cat(
        [
            batch["query_group"],
            torch.full((2, 2), QUERY_GROUP_AREA, dtype=torch.long),
        ],
        dim=1,
    )
    batch["query_mask"] = torch.cat(
    ...
    batch["bbox_upper"] = batch["domain_bbox_upper"]
    batch["bbox_volume"] = torch.tensor([8.0, 8.0], dtype=torch.float32)
    batch["sampling_counts"] = {"global": 2, "containment": 2, "surface_band": 2, "area": 4}
    loss_fn = build_loss_fn(
        {
            "loss": {
                "losses": {
    ...
    metrics = train_step(
        ...
        area_importance_band_width=0.5,
        area_importance_oversample=64,
        area_importance_candidate_chunk_size=16,
    )

    assert metrics["area_importance_band_count"] == pytest.approx(4.0)
    assert metrics["area_importance_candidate_count"] == pytest.approx(256.0)
    assert metrics["area_importance_hit_count"] > 0.0
    assert 0.0 < metrics["area_importance_hit_rate"] < 1.0
    assert 0.0 < metrics["area_importance_volume_mean"] < 8.0
    assert metrics["area_importance_phi_abs_max"] <= 0.5
    assert metrics["area_importance_fallback_count"] == pytest.approx(0.0)
    assert metrics["area_importance_volume"] == pytest.approx(metrics["area_importance_volume_mean"])
